...
| `<maintext>` | 剧情正文（天机正文），会实时渲染到故事区域 | `<maintext>晨曦初露，山门之上雾气氤氲...</maintext>` |
...
| `daoName` | string | — | 道号 |
| `realm` | string | 炼气期/筑基期/金丹期/元婴期/化神期/道祖，可含阶段如 炼气期前期 | 境界 |
| `role` | enum | 掌门/大长老/执法首座/丹峰长老/藏经长老/守山长老/内门弟子/外门弟子/成员 | 职务 |
| `status` | enum | 坐镇/巡查/炼丹/研习/闭关/外出/受伤 | 当前状态 |
| `talent` | string | 甲上/甲中/甲下/乙上/乙中/乙下/丙上/丙中/丙下/丁上/丁中/丁下 | 天赋资质（12 档） |
...
|------|------|--------|------|
| `name` | string | — | 物品名 |
| `type` | enum | 武器/防具/遁具/饰品/丹药/材料/功法/法宝/杂物 | 类型 |
...
<!-- 区域控制权变更 -->
<vars>[{"op":"replace","path":"/world/regions/1/controlledBy","value":"玩家宗门名称"}]</vars>
...
角色三维（杀伐/防御/身法）由天赋资质和境界共同决定。新增成员时建议遵循此规则：

**天赋 12 档排序（高→低）：**
甲上 > 甲中 > 甲下 > 乙上 > 乙中 > 乙下 > 丙上 > 丙中 > 丙下 > 丁上 > 丁中 > 丁下

**计算公式：**

```
base = 50 + (4 - talentIndex) × 3
...
- 乙中 + 练气期中期 = 75
- 乙中 + 筑基期前期 = 500
- 甲上 + 金丹期后期 = round(86 × 3.375 × 100) = 29025
...
若玩家通过开局创建向导启动游戏，以下信息存储在 `_creationMeta` 中，LLM 可通过系统提示词获取：

| 字段 | 说明 |
|------|------|
| `sectName` | 玩家自定义的宗门名称 |
| `path` | 修行法门（道修/神修/魔修/体修/旁门左道） |
| `specialty` | 宗门专长（剑/丹/炼器/法阵/符箓） |
| `crisis` | 开局困境 |
| `personality` | 玩家性格标签 |
| `appearance` | 玩家外貌标签 |
| `age` | 玩家年龄 |

LLM 在生成剧情时应参考这些信息，宗门名称不再是固定的"云璃仙宗"。
...
**星象阁的情报若是属实，这或许是我宗千载难逢的机缘。但九幽教那边，恐怕不会坐视不理。**
...
8. **新增成员时参考天赋/境界计算公式** —— 确保三维数值与设定一致。
9. **宗门名称使用玩家自定义值** —— 通过 `_creationMeta.sectName` 获取，不再使用固定名称。

---

*文档版本：v2.0 | 适用游戏版本：2026-04-18*
