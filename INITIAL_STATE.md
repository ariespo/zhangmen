...
> 游戏未经开局创建向导时的默认状态。若玩家通过创建向导开局，所有数据将被覆盖为玩家自定义值。
...
> 注意：若通过开局创建向导启动，疆域 `controlledBy` 将显示玩家自定义的宗门名称，建筑列表也根据开局地位重新生成。
...
```json
{
  ...
  "gender": "男",
  "age": 612,
  "lifespan": { "current": 612, "max": 1200 },
  "talent": "甲上",
  "stats": { "杀伐": 78, "防御": 68, "身法": 58 },
  "baseStats": { "杀伐": 78, "防御": 68, "身法": 58 },
  "skills": ["太虚真解（第五层）", "云璃剑诀（大成）", "清心咒（圆满）"],
  "personality": ["沉稳", "果决"],
  "appearance": ["一袭青衫", "双目如星"],
  "color": "gold"
}
```

> 若通过开局创建向导启动，player 数据将被玩家自定义值覆盖。
...
{
  "name": "云璃仙宗",
  "foundedYear": "玄黄历三千七百二十一年",
  "location": "东荒云璃山脉主峰",
  "founder": "云璃真人",
  "history": "云璃仙宗创立于玄黄历三千七百二十一年，创派祖师云璃真人以剑入道...",
  "lineage": ["初代：云璃真人（飞升）", "二代：玄霄子（渡劫失败）", "三代：青松道长（寿终正寝）", "四代：明月仙子（失踪）", "五代：清虚子（现任）"],
  "description": "宗门地处东荒云璃山脉，占据九条灵脉交汇之处...",
  "motto": "天道酬勤 · 万法归宗",
  "arrayName": "九曜星辰阵",
  "arrayRank": "地阶上品",
  "arrayDesc": "聚灵护山，攻防一体"
...
> 若通过开局创建向导启动，sect 名称等基础信息将被覆盖，历史和传承由 LLM 根据玩家选择动态生成。
...
```json
[
  { "id": "e1", "text": "苍梧山散修求见，携密信一封，称事关宗门存亡", "time": "刚刚", "location": "苍梧山", "people": "散修", "type": "urgent" },
  { "id": "e2", "text": "成员「林淮安」整理藏经阁时发现一卷失传古籍", "time": "一炷香前", "location": "藏经阁", "people": "林淮安", "type": "normal" },
  { "id": "e3", "text": "天剑宗送来拜帖，邀请参加三月后的论剑大会", "time": "半日前", "location": "天剑宗", "people": "天剑宗使者", "type": "info" },
  { "id": "e4", "text": "炼器峰新铸「碧落剑」一柄，品质上佳，待入库", "time": "一日前", "location": "炼器峰", "people": "炼器长老", "type": "normal" }
]
```

> events 从硬编码迁移至变量系统，支持 LLM 通过 `<vars>` 标签追加新事件。
...
| 字段 | 旧值 | 新值 |
|------|------|------|
| `members.*.talent` | 上上/上/中/下/下下 | 甲上/甲中/甲下/乙上/乙中/乙下/丙上/丙中/丙下/丁上/丁中/丁下 |
| `members.*.realm` | 元婴后期 | 元婴期后期（含"期"字） |
| `members.*.role` | 掌门/大长老/…/成员 | 新增 内门弟子、外门弟子 |
| **新增 `player`** | 无 | 玩家（掌门）自身信息，含姓名、境界、天赋、功法等 |
| **新增 `sect`** | 无 | 宗门信息，含名称、历史、传承、护山大阵等 |
| **新增 `events`** | 无 | 近日要事列表，支持 LLM 追加事件 |
